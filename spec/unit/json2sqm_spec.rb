...
describe Sqm2Json::Reverse, '#to_sqm' do
  ...
  it 'should parse then serialize to SQM many mission files without any single change' do
    sqm_files.each do |sqm_file|
      expect do
        puts "parsing #{sqm_file}"
        json = subject.to_json(File.read(sqm_file))
        sqm = subject.to_sqm(json)

        log_dir = File.join(File.expand_path('../../../log/', __FILE__), File.basename(File.dirname(sqm_file)), 'sqm')
        FileUtils.mkdir_p(log_dir) unless File.exist?(log_dir)
        generated_file_path = File.join(log_dir, File.basename(sqm_file))
        File.delete(generated_file_path) if File.exist?(generated_file_path)
        File.write(generated_file_path, sqm)

        expect(sqm.gsub(/\r\n?/, "\n")).to eq(File.read(sqm_file).gsub(/\r\n?/, "\n"))
